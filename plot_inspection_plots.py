...
def plot_bandpass3(bandpass_frequency, time, bandpass_value, station_names, rows, columns, s_id):
    time = Time(time/60/60/24, format='mjd').value
    a = max(bandpass_frequency) - 10
    b = min(time) + 10
    xx, yy = np.meshgrid(Time(time / 60 / 60 / 24, format='mjd').value, bandpass_frequency)
    ...
    f2.colorbar(c2, cax=f2.add_axes([0.935, 0.155, 0.05, 0.7]))
    f2.tight_layout()
    f2.savefig("results_" + s_id + "/" + "bandpass3_yy.png")
...
def main(solution_file_dir):
    ...
    def run_all_operations():
        ...
        run_process(plot_bandpass3, frequency, time, value, stations_names, rows, columns, sas_id)
